...
export default {
	...
	methods: {
		...
		restart() {
			if(confirm("Are you sure you want to restart? This may mean you need to manually restart your application through the terminal.")) {
				axios
				.post(
					`${process.env.VUE_APP_API_URL}/droplets/${this.dropletID}/actions`, options)
				.then((response) => {
